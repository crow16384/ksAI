...
2.  **Clinical capsule pipeline** — when the study is large,
    exploratory, or token-constrained; decompose, enrich, retrieve, then
    reason over a minimal evidence set.
...
This metadata drives both CSR writing (“describe Week 24 vs Baseline”)
and capsule hierarchy inference.
...
Synthetic SOC capsules are created for section blocks when the source
table uses section headers without explicit `ROW_KIND = SOC`.
...
1.  [`ks_retrieve()`](https://crow16384.github.io/ksAI/reference/ks_retrieve.md)
    → top *n* capsules
2.  Optionally `expand = TRUE` → include child PT capsules (progressive
    disclosure)
...
High-level reasoning happens over SOC summaries; detail rows are fetched
only when the query or `expand` flag requires them.
...
| Many outputs, exploratory Q&A | Capsule | [`as_capsules()`](https://crow16384.github.io/ksAI/reference/as_capsules.md) → [`ks_reason()`](https://crow16384.github.io/ksAI/reference/ks_reason.md) |
...
Set `RUN_LLM <- FALSE` in the script to exercise import, CKR, and
capsule building without a live model.
...
3.  **Traceability** — every capsule and fact links back to
...
5.  **Offline-first** — JSON-only import; local models via Ollama or LM
    Studio.
6.  **Separation of concerns** — structure (R), retrieval (C++23),
    semantics (small LLM), reasoning (large LLM).
