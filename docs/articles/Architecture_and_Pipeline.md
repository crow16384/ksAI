...
ksAI is an offline reasoning layer on top of
[ksTFL](https://crow16384.github.io/ksTFL/) clinical outputs. ksTFL
computes tables, figures, and listings deterministically and writes them
to a **meta folder** as JSON (structural specification plus rendered
data). ksAI reads that folder, compiles outputs into self-contained
**context objects**, and routes them to a language model through two
complementary pipelines:

1.  **Direct context pipeline** — when you already know which outputs
    matter and they fit in the model window.
2.  **Clinical capsule pipeline** — when the study is large,
    exploratory, or token-constrained; an LLM groups tables and figures
    into semantic capsules, then enrich, retrieve, and reason over a
    minimal evidence set.

Both pipelines share the same import layer and the same **CKR**
(Clinical Knowledge Representation) serialization formats. They differ
in how much structure is built before the reasoning model runs.
...
At runtime **no live ksTFL session** is required. Everything ksAI needs
is in the saved JSON artefacts (or in a persisted `.ks` / `.ksc` file).
...
Each spec JSON may contain one or more outputs. ksAI resolves stable
**output ids** (for example `14-3.01`, `14-5.01`) from document
filenames and spec keys.

`ks_list_ids(meta_path)` scans the index and returns id, type, and title
without loading data. `ks_load(meta_path, ids = ...)` parses only the
requested outputs and assembles a `ks_study`.

------------------------------------------------------------------------

## Layer 1: `ks_context` — the render-join

A `ks_context` is the central value object. It is **not** a thin
metadata extract; it is the full render-join of specification and data:
...
- `kind` — for AE tables: `SOC`, `PT`, `OVERALL`; for continuous
  endpoints: `PARAM`

This metadata drives CSR writing (“describe Week 24 vs Baseline”) and
fact-store filtering (`section` / `kind` in
[`as_facts()`](https://crow16384.github.io/ksAI/reference/as_facts.md)).
Capsule trees are built separately by an LLM over whole outputs, not
from row hierarchy.
...
[`as_facts()`](https://crow16384.github.io/ksAI/reference/as_facts.md) /
capsules for structured access to specific rows without sending the full
table.
...
/ `ks_load("file.ks")` persist a compiled subset for reuse without the
original ksTFL folder.

**Design intent:** medical writers work on **targeted subsets** —
efficacy tables for one CSR section, safety tables for another — not
necessarily the entire study at once.
...
Default for reading and for models that benefit from familiar table
layout. Repeats column headers and arm labels per row group. Highest
token cost.

### 3b. `as_compact()` — compact DSL

Pure R. Target: **~50–70% fewer tokens** than markdown for span-heavy
tables.
...
Controlled by `ks_set_option(context_format = "compact")` before
[`ks_llm()`](https://crow16384.github.io/ksAI/reference/ks_llm.md) or
...
`retrieve(facts, rows = ..., sections = ..., spans = ...)` subsets rows
and optionally filters to treatment-arm span columns, then
`as_compact.ks_facts()` renders only the matched slice.
...
accepts either a `ks_study` (creates a per-call chat with
`system_single` prompt) or a `kschat` (reuses session). Placeholders
like `title` and `audience` fill the skill template — they are **not**
passed to the ellmer constructor.
...
**Rule:**
[`ks_chat()`](https://crow16384.github.io/ksAI/reference/ks_chat.md)
embeds every loaded output. Keep the loaded subset small (one or two
tables). For multi-table studies, prefer `ks_llm(study, ...)` or the
capsule pipeline.
...
writes `.md` + `.json`. Pass a prior result to `prior =` in
[`ks_llm()`](https://crow16384.github.io/ksAI/reference/ks_llm.md) to
refine iteratively:
...
Chaining doubles prompt size (table + prior). Use compact tables or
lower `max_rows` when refining locally with 8k-context models.
...
For large studies or exploratory questions, ksAI asks an LLM to group
tables and figures into **clinical capsules** — concept-centric semantic
units — then runs a four-stage agent pipeline over that store.

Membership is by **whole output id** (`member_ids`). Capsules form a
tree via `parent_id` / `child_ids`. Formation is LLM-only: meaning over
titles, excerpts, and figure images — not domain codes, filename
conventions, or row hierarchy.
...
`as_capsules(x, model, ...)` **requires** `model`. It builds a catalog
of Table and Figure contexts (Text is excluded), sends compact excerpts
to the LLM using `inst/prompts/capsule_classify.md`, and validates the
returned JSON into a `ks_capsule_store`.

**What the LLM sees:**

- Titles, subtitles, population, source, and a compact content excerpt
  (`max_excerpt_rows`, `detail = "compact"` or `"full"`)
- For figures: `asset_path` resolved at import; when
  `attach_images = TRUE`, R attaches image bytes via ellmer so a
  **vision** model can read the plot. R does not interpret pixels
  itself.

**Formation rules** (enforced in the prompt and in R validation):

- Group by information meaning only — not domain codes, ICH/CSR
  numbering, or filenames
- Build a tree of themes → sub-themes with stable ASCII `capsule_id`
  slugs
- **Multi-membership** is allowed: one output may support several
  capsules
- Large catalogs are batched (`batch_size`); multiple chunks are merged
  with a second LLM pass
- Capsules below `min_confidence` are dropped; unknown member ids are
  ignored

`n_ctx` cannot be passed to
[`as_capsules()`](https://crow16384.github.io/ksAI/reference/as_capsules.md)
— raise it in LM Studio / Ollama when loading the model.

**Option A — small context** (~8k): shrink prompts; merge pass stitches
the tree.
...
**Option B — large context** (raise Context Length first, e.g. 32k+):
larger batches; better whole-catalog grouping. Vision images only if the
model supports them.
...
Traceability is through `member_ids` back to loaded `ks_context`
objects. Use
[`capsule_content()`](https://crow16384.github.io/ksAI/reference/capsule_content.md)
when you need full member renders rather than the formation excerpts
stored in `compact_text`.

Persist with `save_capsules(store, "study.ksc")` /
[`load_capsules()`](https://crow16384.github.io/ksAI/reference/load_capsules.md).

### Review APIs

Formation and inspection are separate. After
[`as_capsules()`](https://crow16384.github.io/ksAI/reference/as_capsules.md):

| Function | Role |
|----|----|
| `capsule_tree(store)` | Print / return the parent→child tree |
| `capsule_membership(store, study)` | Output id ↔︎ capsule membership table |
...
Two-pass enrichment of capsule labels and compact text — keywords and
optional concepts only:
...
2.  **Optional small-LLM pass** (when `model` is set): structured JSON
    extraction of `concepts`, `synonyms`, `keywords`. A 4B local model
    is sufficient. Use `force = TRUE` to recompute even when keywords
    already exist.
...
calls an OpenAI-compatible `/v1/embeddings` endpoint and stores vectors
in `capsule$embedding`.
...
If embeddings are absent, semantic score falls back to 0 and
keyword/metadata signals still rank results.

### Reasoning agent: `ks_reason()`

Combines retrieval and generation:

1.  [`ks_retrieve()`](https://crow16384.github.io/ksAI/reference/ks_retrieve.md)
    → top *n* capsules
2.  Optionally `expand = TRUE` → include child capsules from the LLM
    tree (progressive disclosure)
...
The reasoning model **never sees the full study** — only retrieved
capsule summaries. This is the capsule pipeline’s primary token
advantage.
...
High-level reasoning happens over parent capsule summaries; child themes
are fetched only when the query or `expand` flag requires them. For full
member table/figure text outside reasoning, use
[`capsule_content()`](https://crow16384.github.io/ksAI/reference/capsule_content.md).
...
ksAI uses [ellmer](https://ellmer.tidyverse.org/) for provider
abstraction.
...
|----|----|----|
| Capsule formation / annotation / review | 4B+ (vision if figures) | [`as_capsules()`](https://crow16384.github.io/ksAI/reference/as_capsules.md), `ks_annotate(..., model = ...)`, [`ks_review_capsules()`](https://crow16384.github.io/ksAI/reference/ks_review_capsules.md) |
...
Unload large models before loading small ones in LM Studio when memory
is constrained.
...
[`ks_chat()`](https://crow16384.github.io/ksAI/reference/ks_chat.md) —
the chat system prompt is built at construction time.
...
| Many outputs, exploratory Q&A | Capsule | `as_capsules(..., model)` → [`ks_reason()`](https://crow16384.github.io/ksAI/reference/ks_reason.md) |
| Large AE / lab table, token limit | Capsule or facts | [`ks_retrieve()`](https://crow16384.github.io/ksAI/reference/ks_retrieve.md) or [`retrieve()`](https://crow16384.github.io/ksAI/reference/retrieve.md) |
| Iterative refinement | Direct + chain | [`save_result()`](https://crow16384.github.io/ksAI/reference/save_result.md) → `prior = load_result()` |
...
|----|----|----|
| `.ks` | Compiled `ks_study` (embedded rows) | [`save_study()`](https://crow16384.github.io/ksAI/reference/save_study.md), [`ks_load()`](https://crow16384.github.io/ksAI/reference/ks_load.md) |
| `.ksc` | Capsule store (keywords, embeddings) | [`save_capsules()`](https://crow16384.github.io/ksAI/reference/save_capsules.md), [`load_capsules()`](https://crow16384.github.io/ksAI/reference/load_capsules.md) |
| `.json` / `.md` | `ks_result` from LLM runs | [`save_result()`](https://crow16384.github.io/ksAI/reference/save_result.md), [`load_result()`](https://crow16384.github.io/ksAI/reference/load_result.md) |
| ksTFL `meta/` | Source spec + data JSON | [`ks_list_ids()`](https://crow16384.github.io/ksAI/reference/ks_list_ids.md), [`ks_load()`](https://crow16384.github.io/ksAI/reference/ks_load.md) |
...
A full walkthrough of all four workflows (direct, facts, capsule,
...
Set `RUN_LLM <- FALSE` in the script to exercise import and CKR without
live chat/embed calls. Capsule formation (`as_capsules`) always requires
a model.
...
1.  **Deterministic upstream** — ksTFL numbers are final; ksAI never
    recomputes statistics.
2.  **Targeted loading** — load only the ids you need; scale to full
    studies via capsules.
3.  **Traceability** — every capsule links to member output ids; facts
    retain row structure from `ks_context`.
4.  **Token economy** — compact DSL, fact filtering, and capsule
    retrieval minimize model input.
5.  **Offline-first** — JSON-only import; local models via Ollama or LM
    Studio.
6.  **Separation of concerns** — capsule formation (LLM over catalog +
    vision), enrichment (R + optional small LLM), retrieval
    (embeddings + keywords), reasoning (large LLM).

These layers compose: you can use
[`as_compact()`](https://crow16384.github.io/ksAI/reference/as_compact.md)
without capsules, capsules without embeddings (keyword-only retrieval),
or the full stack for large-study Q&A.
