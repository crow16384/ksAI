...
`ks_capsule_store`. Domain is inferred **once per output** (not per capsule).

**Domain inference** (language-agnostic; first hit wins):

1. `enrich_context(..., annotations = list(domain = ...))`
2. Session `domain_map` (`ks_set_option()`) — exact id or regex
...
7. Optional small LLM when still `UNKNOWN` (`llm_domain = "unknown"`, default)
8. `"UNKNOWN"`
...
2. **Optional small-LLM pass**: structured JSON extraction of `concepts`,
   `synonyms`, `keywords`. A 4B local model is sufficient.
3. **Domain fallback** (when `model` is set): reclassify capsules still tagged
   `UNKNOWN`, once per `source_id`. Use `force_domain = TRUE` to reclassify
   every table.
...
| `domain_map` | `character()` | Named id/regex → domain overrides for capsules |
