...
`n_ctx` cannot be passed to `as_capsules()` — raise it in LM Studio / Ollama
when loading the model.

**Option A — small context** (~8k): shrink prompts; merge pass stitches the tree.
...
**Option B — large context** (raise Context Length first, e.g. 32k+): larger
batches; better whole-catalog grouping. Vision images only if the model supports them.
