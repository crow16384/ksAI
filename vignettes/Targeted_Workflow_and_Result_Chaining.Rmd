...
Domain codes are inferred once per output (multilingual rules; optional small
LLM). Hard overrides win first: `enrich_context(annotations$domain)` and
...
richer keywords. The same `model` also reclassifies capsules still tagged
`UNKNOWN` (once per `source_id`). Use `force_domain = TRUE` to reclassify
every table. LM Studio with `qwen3.5-4b` or similar works well:
...
The concept response is expected to be JSON with keys `concepts`, `synonyms`,
`keywords`. Domain responses use `{"domain":"CODE","confidence":0.0}`.
Any non-JSON wrapper text is stripped automatically.
...
2. `as_capsules(study)` → concept-level decomposition
   (optional `model` / `llm_domain` for UNKNOWN domains)
